...
class IntentAgentFactory:
    ...
    def create(settings: Settings = None) -> IntentAgent:
        ...
        ai_strategy._repo = repository  # injeta repo para carregar tabelas relacionadas
    ...
    def _create_repository(settings: Settings):
        ...
        if not settings.use_firestore:
            raise ValueError("❌ Firestore está desabilitado. Configure USE_FIRESTORE=true no .env")
        
        # Modo Firestore: usar apenas Firebase (RAG de dados)
