...
class IntentAgent:
    ...
    def scan_and_process(self, user_prompt: str, flow_id: str) -> IntentObject:
        ...
        result = self.ai_strategy.ai_service.infer_scan(user_prompt, flow_id, catalog)
    ...
    def _build_intent_from_scan(self, user_prompt: str, flow_id: str,
        ...
        # Carregar configuração do database do Firestore
        database_name = flow_id  # Fallback: usar flow_id
        database_type = "SYBASE"  # Fallback padrão
        
        try:
            # Tentar carregar do documento _config primeiro
            config_doc = self.repo.db.collection(flow_id).document("_config").get()
            if config_doc.exists:
                config_data = config_doc.to_dict() or {}
                if 'database' in config_data:
                    db_info = config_data['database']
                    database_name = db_info.get('name', flow_id)
                    database_type = db_info.get('type', 'SYBASE')
                    print(f"   📋 Database configurado: {database_name} ({database_type})")
            else:
                # Fallback: tentar carregar de qualquer documento da coleção
                docs = list(self.repo.db.collection(flow_id).limit(1).stream())
                if docs:
                    doc_data = docs[0].to_dict() or {}
                    if 'flow_metadata' in doc_data:
                        flow_meta = doc_data['flow_metadata']
                        database_name = flow_meta.get('domain', flow_id)
                        database_type = flow_meta.get('type', 'SYBASE')
        except Exception as e:
            print(f"   ⚠️  Usando flow_id como database_name: {e}")
        ...
        return IntentObject(
            ...
            metadata={
                ...
                "gemini_reasoning": ai_result.get("reasoning", "")
            },
    ...
    def _build_catalog(self, tables: List[Dict]) -> List[Dict]:
        """
        Monta catálogo completo: nome + schema + colunas + FKs.
            ...
            columns = [
                {
                    "name": c.get("name", ""),
                    "type": c.get("type", ""),
                    "full_ref": f"{schema}.{table_name}.{c.get('name', '')}" if schema else f"{table_name}.{c.get('name', '')}",
                }
                for c in t.get("columns_dictionary", [])
            ]
            # FKs da tabela (se disponíveis no original_data)
            table_def = t.get("original_data", {}).get("table_definition", {})
            fks = table_def.get("constraints", {}).get("foreign_keys", [])
